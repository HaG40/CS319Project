...
function LandingPage() {
  ...
  const loadUserInfo = async () => {
    if (!user?.id) return
    try {
      const res = await axios.get(`http://localhost:3000/api/act/user-info/${user.id}`)
      const data = res.data
      ...
      console.log("ไม่พบข้อมูลผู้ใช้ก่อนหน้า")
    }
  }
  ...
  const submitApplication = async () => {
    try {
      ...
      if (!fullname || !email || !phone || !line || !age) {
                ...
                <li key={activity.id}>
                  ...
                  <div className="border-b h-60 rounded-b-2xl px-4 pb-4 pt-2 flex flex-col shadow-lg border-gray-200 bg-white">
                    ...
                    <p className="text-sm text-gray-700">
                      <b className="text-emerald-600">รายละเอียด:</b> {activity.description}
                    </p>
                    <p className="text-sm text-gray-700">
                      <b className="text-emerald-600">ประเภท:</b> {activity.category}
                    </p>

                    <p className="text-sm text-gray-600">
                      <b className="text-emerald-600">เริ่ม:</b> {formatDate(activity.start_date)}
                    </p>
                    <p className="text-sm text-gray-600">
                      <b className="text-emerald-600">สิ้นสุด:</b> {formatDate(activity.end_date)}
                    </p>
                    ...
                    <div className="mt-auto flex justify-between items-center">
                      ...
                      <div className="flex flex-col gap-0.5">
                        ...
                        <button
                          ...
                          className={`p-2 rounded text-white cursor-pointer text-sm ${
                            hasUserJoined(activity)
                              ? "bg-gray-400 cursor-not-allowed"
                              : "bg-emerald-500"
                          }`}
                          disabled={hasUserJoined(activity)}
                        >
                          {hasUserJoined(activity) ? "สมัครแล้ว" : "เข้าร่วม"}
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      {isOpen && selectedActivity && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl w-96">
            <h2 className="text-xl font-bold mb-4 text-emerald-600">สมัครเข้าร่วมกิจกรรม</h2>
            ...
            <div className="flex flex-col gap-2.5 mb-5">
              ...
              <p className="text-xs text-gray-500">
                *** ข้อมูลติดต่อเหล่านี้ไว้สำหรับการติดต่อผู้สมัครเข้าร่วม
              </p>
            </div>
